...
conv_layer_t* make_conv_layer(int in_sx, int in_sy, int in_depth,
  ...
  // computed
  l->out_sx = floor((l->in_sx -1) + 1);
  l->out_sy = floor((l->in_sy -1) + 1);

  l->filters = (vol_t**)malloc(sizeof(vol_t*)*filters);
  for (int i = 0; i < filters; i++) {
    l->filters[i] = make_vol(5, 5, l->in_depth, 0.0);
  }
...
void conv_forward_d3(conv_layer_t* l, vol_t** in, vol_t** out, int start, int end) {
            ...
            for(int fy = 0; fy < 5; fy++) {
              ...
              for(int fx = 0; fx < 5; fx++) { 
                ...
                if(oy >= 0 && oy < 32 && ox >=0 && ox < 32) {
                  a += fw[((5 * fy)+fx)*3] * V_w[((32 * oy)+ox)*3];
                  a += fw[((5 * fy)+fx)*3+1] * V_w[((32 * oy)+ox)*3+1];
                  a += fw[((5 * fy)+fx)*3+2] * V_w[((32 * oy)+ox)*3+2];
                }
              }
            }
            a += addtoa;
...
void conv_forward_d16(conv_layer_t* l, vol_t** in, vol_t** out, int start, int end) {
            ...
            for(int fy = 0; fy < 5; fy++) { 
              ...
              for(int fx = 0; fx < 5; fx++) { 
                ...
                if(oy >= 0 && oy < 16 && ox >=0 && ox < 16) {
                  ...
                  int A_sub = ((5 * fy)+fx)*16;
                  int B_sub = ((16 * oy)+ox)*16;

                  for(int fd = 0;fd < 16 ; fd += 8) { 

                    __m256d A = _mm256_loadu_pd(fw + A_sub + fd);
                    __m256d B = _mm256_loadu_pd(V_w + B_sub + fd);
                    sum = _mm256_add_pd(sum, _mm256_mul_pd(A, B));

                    A = _mm256_loadu_pd(fw + A_sub + fd + 4);
                    B = _mm256_loadu_pd(V_w + B_sub + fd + 4);
                    sum = _mm256_add_pd(sum, _mm256_mul_pd(A, B));

                  }
                    double arr[4];
                    _mm256_storeu_pd(arr, sum);
                    for (int i = 0; i < 4; i++) {
                      a += arr[i];
                  }
                }
...
void conv_forward_d20(conv_layer_t* l, vol_t** in, vol_t** out, int start, int end) {
            ...
            for(int fy = 0; fy < 5; fy++) { 
              ...
              for(int fx = 0; fx < 5; fx++) { 
                ...
                if(oy >= 0 && oy < 8 && ox >=0 && ox < 8) {
                  __m256d sum = _mm256_setzero_pd();
                  int A_sub = ((5 * fy)+fx)*20;
                  int B_sub = ((8 * oy)+ox)*20;
                  for(int fd = 0;fd < 20; fd += 4) { 
                    __m256d A = _mm256_loadu_pd(fw + A_sub + fd);
                    __m256d B = _mm256_loadu_pd(V_w + B_sub+fd);
                    sum = _mm256_add_pd(sum, _mm256_mul_pd(A, B));
                  }
...
network_t* make_network() {
  ...
  net->v[3] = make_vol(16, 16, 16, 0.0);
  net->l3 = make_conv_layer(16, 16, 16, 5, 20, 1, 2);
  ...
  net->v[6] = make_vol(8, 8, 20, 0.0);
  net->l6 = make_conv_layer(8, 8, 20, 5, 20, 1, 2);
...
// Timestamp
double preloop_time = 0;

double layer0_time = 0;
double layer1_time = 0;
double layer2_time = 0;
double layer3_time = 0;
double layer4_time = 0;
double layer5_time = 0;
double layer6_time = 0;
double layer7_time = 0;
double layer8_time = 0;
double layer9_time = 0;
double layer10_time = 0;
...
void net_forward(network_t* net, batch_t* v, int start, int end) {
  preloop_time = (double)timestamp_us();
  double temp_timeA = 0;
  double temp_timeB = 0;

  conv_forward_d3(net->l0, v[0], v[1], start, end);
  temp_timeA = (double)timestamp_us();
  layer0_time += (temp_timeA - preloop_time);

  relu_forward(net->l1, v[1], v[2], start, end);
  temp_timeB = (double)timestamp_us();
  layer1_time += (temp_timeB - temp_timeA); 

  pool_forward(net->l2, v[2], v[3], start, end);
  temp_timeA = (double)timestamp_us();
  layer2_time += (temp_timeA - temp_timeB);

  conv_forward_d16(net->l3, v[3], v[4], start, end);
  temp_timeB = (double)timestamp_us();
  layer3_time += (temp_timeB - temp_timeA); 

  relu_forward(net->l4, v[4], v[5], start, end);
  temp_timeA = (double)timestamp_us();
  layer4_time += (temp_timeA - temp_timeB);

  pool_forward(net->l5, v[5], v[6], start, end);
  temp_timeB = (double)timestamp_us();
  layer5_time += (temp_timeB - temp_timeA);

  conv_forward_d20(net->l6, v[6], v[7], start, end); 
  temp_timeA = (double)timestamp_us();
  layer6_time += (temp_timeA - temp_timeB);

  relu_forward(net->l7, v[7], v[8], start, end);
  temp_timeB = (double)timestamp_us();
  layer7_time += (temp_timeB - temp_timeA);

  pool_forward(net->l8, v[8], v[9], start, end);
  temp_timeA = (double)timestamp_us();
  layer8_time += (temp_timeA - temp_timeB);

  fc_forward(net->l9, v[9], v[10], start, end);
  temp_timeB = (double)timestamp_us();
  layer9_time += (temp_timeB - temp_timeA);

  softmax_forward(net->l10, v[10], v[11], start, end);
  temp_timeA = (double)timestamp_us();
  layer10_time += (temp_timeA - temp_timeB);
}
...
void net_classify_cats(network_t* net, vol_t** input, double* output, int n) {
  ...
  printf("LAYER 0 TIME: %f\n", (double) layer0_time / 1000);
  printf("LAYER 1 TIME: %f\n", (double) layer1_time / 1000);
  printf("LAYER 2 TIME: %f\n", (double) layer2_time / 1000);
  printf("LAYER 3 TIME: %f\n", (double) layer3_time / 1000); 
  printf("LAYER 4 TIME: %f\n", (double) layer4_time / 1000); 
  printf("LAYER 5 TIME: %f\n", (double) layer5_time / 1000);
  printf("LAYER 6 TIME: %f\n", (double) layer6_time / 1000);  
  printf("LAYER 7 TIME: %f\n", (double) layer7_time / 1000); 
  printf("LAYER 8 TIME: %f\n", (double) layer8_time / 1000); 
  printf("LAYER 9 TIME: %f\n", (double) layer9_time / 1000);  
  printf("LAYER 10 TIME: %f\n", (double) layer10_time / 1000);
