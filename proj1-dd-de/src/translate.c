...
unsigned write_pass_one(FILE* output, const char* name, char** args, int num_args) {
  if (strcmp(name, "li") == 0) {
    ...
    if (imm >= -32768 && imm < 32768) {
      if (imm < 0) {
        int imm_up = (~(imm - 1)) & -65536;
        int imm_low = imm & 65535;
        imm = imm_up | imm_low;
      }
      fprintf(output, "addiu %s $0 %ld\n", args[0], imm);
      return 1;
    } else {
      int imm_lo = imm & 65535;
      int imm_hi =  imm & -65536; 
      imm_hi >>= 16;
      imm_hi = imm_hi &65535;
...
int translate_inst(FILE* output, const char* name, char** args, size_t num_args, uint32_t addr,
  ...
  /* This part matches each instruction with the MIPS function and returns the corresponding
      function with the appropriate opcode/funct. There should only be 18 possible entries.*/
...
int write_rtype(uint8_t funct, FILE* output, char** args, size_t num_args) {
    ...
    if (rd == -1 || rs == -1 || rt == -1 || funct < 0 || funct > 64) {
      return -1;
    }
    ...
    instruction = opcode | rd | rs | rt | shift | funct; 
...
int write_shift(uint8_t funct, FILE* output, char** args, size_t num_args) {
    ...
    if (rd == -1 || rt == -1 || err == -1 || funct < 0 || funct > 64) {
      return -1;
    }
    ...
    instruction = opcode | rd | rs | rt | shamt | funct;
    write_inst_hex(output, instruction);
    return 0;
}
...
int write_mem(uint8_t opcode, FILE* output, char** args, size_t num_args) {
  ...
  int err = translate_num(&offset, args[1], -32768, 65535);
  uint32_t instruction = 0;
  ...
  instruction = opcode1 | rs | rt | offset;
  write_inst_hex(output, instruction);
  return 0;
}
...
int write_branch(uint8_t opcode, FILE* output, char** args, size_t num_args, 
  ...
  int rt = translate_reg(args[1]);
  char* label = args[2];
  if (get_addr_for_symbol(symtbl, label) == -1) {
    return -1;
  }
  int label_addr = -(((addr + 4)-(get_addr_for_symbol(symtbl, label)))/4);
  uint32_t instruction = 0;

  if (rs == -1 || rt == -1 || label_addr < -32768 || label_addr > 32768 ) {
    return -1;
  }
  int opcode1 =  opcode;
  opcode1 <<= 26; 
  ...
  label_addr = label_addr & 65535;
  instruction = opcode1 | rs | rt | label_addr;
...
int write_jump(uint8_t opcode, FILE* output, char** args, size_t num_args,  
  ...
  if (label_addr < -33554432 || label_addr > 33554432 ) {
    return -1;
  }
  int opcode1 = opcode;
  opcode1 <<= 26; 
  instruction = opcode1 | label_addr;
  write_inst_hex(output, instruction);
  return 0;
}