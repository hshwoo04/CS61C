...
# so if necessary put leading 0s in the buffer. Therefore, you should always 
...
hex_to_str:
	...
	addi $t0, $0, 0   # t0 = '\0'
	sb $t0, 9($a1)     # store '\0' as last slot
	addi $t0, $0, 10  # t0 = '\n'
	sb $t0, 8($a1)     # store '\n' as second to last slot
	addi $t3, $a1, 7     
	addi $t4, $zero, 2    #counter
	addi $t5, $zero, 48
...
loopHex:
        ...
        srl $a0, $a0, 4       # 16 
        slti $t2, $t0, 10     
        addi $t1, $t0, 48     # add 48.
        bne $t2, $0, store     # skip the add 39
        addi $t1, $t1, 39      
store:
        sb $t1, 0($t3)        
        addi $t3, $t3, -1
