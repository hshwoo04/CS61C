...
#------------------------------------------------------------------------------
addr_for_symbol:
	beq $a0, $0, addr_not_found	# Begin symbol_for_addr
	addi $sp, $sp, -12
	sw $ra, 8($sp) # store register address
	sw $a1, 4($sp)  # store name to look for
	sw $a0, 0($sp)  # store pointer to SymbolList

	lw $t0, 4($a0) # load name of current SymbolList
	
	
	add $a0, $t0, $zero
	jal streq      # run streq to see if they're equal
	...
	beq $v0, $zero, addr_found #if return variable from streq is $0, we done

	lw $t0, 8($a0) #list = list.next;
	add $a0, $t0, $zero
	j addr_for_symbol
...
add_to_list:	
	...
	sw $s1, 4($sp)
	sw $s0, 0($sp)
	move $s0, $v0 # s0: pointer to new node
	move $a3, $a0 # a3: pointer to list
	move $a0, $a2 # a0: pointer to name of symbol
	...
	move $s1, $v0 # s1: copy of string
	sw $a1, 0($s0)
	sw $s1, 4($s0)
	sw $0, 8($s0)
	beq $a3, 0, null_case
	bne $a3, 0, not_null_case
...
not_null_case:
	move $t0, $a3 # store pointer to list in $t0
	move $a3, $s0
	sw $t0, 8($s0)
	# sw $t0, 0($s0)
	# sw  $s0, 0($a3)
