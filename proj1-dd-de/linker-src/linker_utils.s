...
inst_needs_relocation:
	...
	andi $t0, $a0, 4227858432
	srl $t0, $t0, 26
	beq $t0, 2, needs_relocation # checks whether the opcode is jump (0010)
	beq $t0, 3, needs_relocation # checks whether the opcode of jal (0011)
	
	li $v0, 0 # no need for relocation
	jr $ra

needs_relocation:
	li $v0, 1 # needs to relocate
	jr $ra
...
relocate_inst:
	...
	move $a0 $s3
	move $a1 $s1 # use relocation table

	jal symbol_for_addr

	beq $v0, 0, symbol_error # symbol check
	move $s4 $v0 # the symbol in s4
	...
	sw $s3 12($sp)
	sw $s4 16($sp)
	jal addr_for_symbol # find relocated address
	lw $s0 0($sp)
	lw $s1 4($sp)
	lw $s2 8($sp)
	lw $s3 12($sp)
	...
	addi $sp $sp 20
	beq $v0, -1, addr_error
	move $s5, $v0 # relocated address in s5

	andi $s5, $s5, 268435455 # gets rid of the top 4 bits of addr
	srl  $s5, $s5, 2 # gets rid of bottom 2 bits of addr
	# sth srong with s5

	srl $s0, $s0, 26 # using AND to get the new instruction
	sll $s0, $s0, 26
	or $s0, $s0, $s5
	move $v0 $s0
	j relocate_finish
