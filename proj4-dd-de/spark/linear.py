...
class LinearClassifier(Classifier):
  ...
  def backward(self, data, count):
    ...
    Hint: 5You need to reduce the RDD from 'softmax loss layer'
    """ 
    ...
    """
    dLdA = softmax.map(lambda (x, (L, df)) : linear_backward(df, x, self.A)).map(lambda (d, e, f) : e).reduce(lambda a, b : a + b)
    dLdb = softmax.map(lambda (x, (L, df)) : linear_backward(df, x, self.A)).map(lambda (d, e, f) : f).reduce(lambda a, b : a + b)
